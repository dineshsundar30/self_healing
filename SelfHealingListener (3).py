...
1. Run Keyword And Ignore Error / Continue On Failure / Return Status / Expect Error
   → caller intentionally ignores/handles the error — healing would be wrong
...
NOTE: 'Wait Until Keyword Succeeds' is NOT suppressed — it retries until
success, so healing should absolutely fire inside it.
"""
...
# Printed exactly once across the entire run (class-level flag)
...
class SelfHealingListener:
    ...
    # Only true "ignore error" wrappers — NOT Wait Until Keyword Succeeds
    # (that keyword retries until success, healing must work inside it)
    ...
    def start_suite(self, data, result):
        ...
        if not _ENGINE_BANNER_PRINTED:
            ...
            print(
                f"\033[32m\033[1m[SELF-HEAL] Engine ACTIVE — "
                f"WebDriver.find_element patched globally."
                f"\033[0m",
                flush=True
            )
    ...
    def _patch_webdriver(self):
        ...
        def healed_find_element(driver_self, by='id', value=None):
            ...
            # STEP 3 — suppression (no I/O, immediate)
            suppressed, suppress_reason = listener_ref._get_suppression()
            if suppressed:
                failure_counts[value_str] += 1
                listener_ref._log(
                    f"<span style='color:#999'>[SELF-HEAL] SUPPRESSED &mdash; "
                    f"{suppress_reason} | "
                    f"<code>{value_str[:100]}</code></span>",
                    "INFO", html=True)
                raise captured_error

            # STEP 4 — rate-limit (no I/O, immediate)
            now = time.time()
            ...
            # STEP 5 — count + variable name (cached)
            failure_counts[value_str] += 1
            ...
            bt = f"PERSISTENT FAIL #{fail_count}" if is_persistent else f"FAIL #{fail_count}"
            ...
            # STEP 7 — fetch page source (only when healing actually runs)
            try:
